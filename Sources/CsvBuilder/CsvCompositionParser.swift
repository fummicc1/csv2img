...
public struct CsvCompositionParser {
    ...
    public static func parse<Composition: CsvComposition>(
        ...
        let compositionType = String(describing: type)
        guard let file = Bundle.main.url(forResource: compositionType, withExtension: "txt") else {
            throw Error.fileNotFound(type: compositionType)
        }
        let content = try Data(contentsOf: file)
        guard let source = String(data: content, encoding: .utf8) else {
            throw Error.failedToDecodeWithUtf8
        }
        let syntax: SourceFileSyntax = Parser.parse(source: source)
        return parseIntoCsv(type: type, source: syntax, rows: rows)
    }
    ...
    static func extractColumns<C: CsvComposition>(_ type: C.Type, variableDecl decl: VariableDeclSyntax) -> [String] {
        ...
        return attributes.compactMap { attribute in
            ...
            for attr in attributes {
                ...
                let column = tokens
                    .compactMap {
                        if case let TokenKind.stringSegment(column) = $0.tokenKind {
                            return column
                        }
                        return nil
                    }
...
extension StructDeclSyntax {
    func findColumns(type: (some CsvComposition).Type, members: MemberDeclListSyntax) -> [String] {
        ...
        for member in members {
            ...
            let c = CsvCompositionParser.extractColumns(type, variableDecl: decl)
            if c.isEmpty {
                continue
            }
            columns.append(contentsOf: c)
        }
        return columns
    }
}
